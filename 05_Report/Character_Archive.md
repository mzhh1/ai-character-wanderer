# Character Archive: The Wanderer (Scaramouche)

This archive serves as the central index for the "Virtual Character Analysis" assignment deliverables.

## 1. Character Profile & CV (Setting Sheets)
> Detailed character background, psychological profile, MBTI, and experiment logs.

*   📄 **[View Full Experiment Report & Profile](report.md)**

## 2. Dialogs & Interaction Logs (Text Generation)

### 2.1 Experiment Logs: Human-in-the-Loop Correction
> Records of the "Persona Correction" process. Includes trap questions, AI failures, and human interventions to restore character consistency.

*   🧪 **[Phase 1: Profile Approach](../02_Text_Gen/experiment_dialog/dialog1.md)** (Stereotype Trap)
*   🧪 **[Phase 2: Narrative Approach](../02_Text_Gen/experiment_dialog/dialog2.md)** (Hallucination Trap)
*   🧪 **[Phase 3: CoT](../02_Text_Gen/experiment_dialog/dialog3(cot).md)** (Chain of Thought Attempt)

### 2.2 Final Results: Stable Workflow Output
> 3 rounds of stable conversation generated using the final "Modular Workflow" (Analyst -> Inner Self -> Speaker), demonstrating long-term consistency.

*   ✅ **[Conversation 1: Virtual Scene](../02_Text_Gen/final_result/conversation1.md)**
*   ✅ **[Conversation 2: Outlook On Life](../02_Text_Gen/final_result/conversation2.md)**
*   ✅ **[Conversation 3: Daily Interaction](../02_Text_Gen/final_result/conversation3.md)**

## 3. Lifestyle Photo Series (Image Generation)
> Photos of the character in 5 different scenarios (Dining, Scholar, Combat, Sleeping, Night).

*   🖼️ **[Text-to-Image Gallery](../03_Image_Gen/text_to_image.md)** (Txt2Img Results)
*   🖼️ **[Image-to-Image Gallery](../03_Image_Gen/image_to_image.md)** (Img2Img Optimization)

## 4. Theme Song (Audio/Music Generation)
> Character theme songs generated based on personality traits.

*   🎵 **[Audio Archive](../04_Audio_Video_Gen/audio.md)**
    *   *The Ballad of the Wanderer (Theme)*
    *   *The False God's Echo (Villain Theme)*
    *   *Ashes and Wind*
    *   *Deus Adefic*
    *   *Fugitive in Sumeru*

## 5. Visual Script (Video Generation)
> Script description used for video generation.

*   🎬 **[Video](../04_Audio_Video_Gen/video.md)**
